		...
		<div class="couponList">
			...
			<table class="table table-hover" style="text-align: center;">
				...
				<tr>
					...
					<td>${row.exp_date}</td>
					<td>
						<c:if test="${row.c_no == 1}">PREP LIVE IN SEOUL</c:if>
						<c:if test="${row.c_no == 2}">Johnny Stimson Live in Seoul</c:if>
						<c:if test="${row.c_no == 3}">LIVE IN SEOUL JOHN K</c:if>
						<c:if test="${row.c_no == 4}">Maksim Mrvica - The Collection</c:if>
						<c:if test="${row.c_no == 5}">Maksim Mrvica - The Collection</c:if>
